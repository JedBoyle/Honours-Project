...
// TODO make the classifier a DzClassifier
class DzCARTTrainer: public AbstractTrainer<CARTClassifier<RealVector>, unsigned int> , public AbstractTrainer<CARTClassifier<RealVector>, RealVector > {
public:
    typedef CARTClassifier<RealVector> ModelType;

    /// Constructor
    DzCARTTrainer(std::size_t _m_nodeSize = 1, std::size_t _m_numberOfFolds = 10, std::size_t _m_treeSize = -1):
        m_nodeSize(_m_nodeSize),
        m_numberOfFolds(_m_numberOfFolds) {
            if(_m_treeSize == -1) {
                m_treeSize = std::numeric_limits<std::size_t>::max();
            }
            else {
                m_treeSize = _m_treeSize;
            }
    }

    /// \brief From INameable: return the class name.
    std::string name() const
    { return "DzCARTTrainer"; }
    ...
    void setNodeSize(std::size_t _m_nodeSize) {
        m_nodeSize = _m_nodeSize;
    }

    void setTreeSize(std::size_t _m_treeSize) {
        m_treeSize = _m_treeSize;
    }

    /*int getSize() {
        return splitMatrix.size();
    }*/
...
protected:
    ...
    ///Types frequently used
     ...
     ///Controls the maximim number of nodes in the tree
