...
//TODO need to check the the buildtree and train methods work for regression models.
// because  added checks that limit the size of the tree.
//TODO delete these imports in final build
#include <iostream>
...
void DzCARTTrainer::train(ModelType& model, RegressionDataset const& dataset)
{
    ...
    for (unsigned fold = 0; fold < m_numberOfFolds; ++fold){
        ...
        while(splitMatrix.size()!=1){
            ...
            if(error < bestErrorRate && splitMatrix.size() <= m_treeSize){
...
//Classification
void DzCARTTrainer::train(ModelType& model, ClassificationDataset const& dataset){
    //Store the number of input dimensions
    m_inputDimension = inputDimension(dataset);
    ...
    // Prune the tree until it the correct size.
    int i = bestSplitMatrix.size() - 1;
    while(bestSplitMatrix.size() > m_treeSize) {
        bestSplitMatrix.erase(bestSplitMatrix.begin()+i);
        --i;
    }
...
//TODO
//~ // train using weights
//~ void DzCARTTrainer::train(ModelType& model, ClassificationDataset const& dataset, RealVector const& weights, double& error) {
    //~ //Store the number of input dimensions
    //~ m_inputDimension = inputDimension(dataset);
...
void DzCARTTrainer::pruneMatrix(SplitMatrixType& splitMatrix){
...
std::size_t DzCARTTrainer::findNode(SplitMatrixType& splitMatrix, std::size_t nodeId){
    unsigned int i = 0;
...
*/
void DzCARTTrainer::pruneNode(SplitMatrixType& splitMatrix, std::size_t nodeId){
...
void DzCARTTrainer::measureStrenght(SplitMatrixType& splitMatrix, std::size_t nodeId, std::size_t parentNode){
...
//Classification case
DzCARTTrainer::SplitMatrixType DzCARTTrainer::buildTree(AttributeTables const& tables, ClassificationDataset const& dataset, boost::unordered_map<std::size_t, std::size_t>& cAbove, std::size_t nodeId ){
    ...
    //if(!(gini(cAbove,n)==0 || n <= m_nodeSize || m_treeSize <= nodeId)){
...
RealVector DzCARTTrainer::hist(boost::unordered_map<std::size_t, std::size_t> countMatrix){
...
DzCARTTrainer::SplitMatrixType DzCARTTrainer::buildTree(AttributeTables const& tables, RegressionDataset const& dataset, std::vector<RealVector> const& labels, std::size_t nodeId, std::size_t trainSize){
    ...
    n = tables[0].size();
    //if(n > m_nodeSize && nodeId <= m_treeSize) {
    if(n > m_nodeSize && nodeId <= m_treeSize) {
        //label vectors
 ...
 */
RealVector DzCARTTrainer::mean(std::vector<RealVector> const& labels){
 ...
 */
double DzCARTTrainer::totalSumOfSquares(std::vector<RealVector> const& labels, std::size_t start, std::size_t length, const RealVector& sumLabel){
    if (length < 1)
        throw SHARKEXCEPTION("[DzCARTTrainer::totalSumOfSquares] length < 1");
    if (start+length > labels.size())
        throw SHARKEXCEPTION("[DzCARTTrainer::totalSumOfSquares] start+length > labels.size()");
 ...
 */
void DzCARTTrainer::splitAttributeTables(AttributeTables const& tables, std::size_t index, std::size_t valIndex, AttributeTables& LAttributeTables, AttributeTables& RAttributeTables){
...
double DzCARTTrainer::gini(boost::unordered_map<std::size_t, std::size_t>& countMatrix, std::size_t n){
    double res = 0;
    boost::unordered_map<std::size_t, std::size_t>::iterator it;
 ...
 */
DzCARTTrainer::AttributeTables DzCARTTrainer::createAttributeTables(Data<RealVector> const& dataset){
...
boost::unordered_map<std::size_t, std::size_t> DzCARTTrainer::createCountMatrix(ClassificationDataset const& dataset){
